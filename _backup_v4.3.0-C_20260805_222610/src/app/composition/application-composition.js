import {
  LOCAL_DEMO_USER,
} from '../../platform/auth/local-auth-service.js';
...
export class ApplicationComposition {
  ...
  renderAuthenticatedApplication(
    ...
    const routeView =
      this.requireElement('#route-view');
        ...
        >
          ...
          <aside class="demo-credentials">
            ...
            <dl>
              <div>
                <dt>E-mail</dt>
                <dd>${LOCAL_DEMO_USER.email}</dd>
              </div>

              <div>
                <dt>Senha</dt>
                <dd>${LOCAL_DEMO_USER.password}</dd>
              </div>
            </dl>

            <p>
              Esta autenticação é apenas para validar
              a experiência local. Ainda não utiliza
              servidor nem Supabase.
            </p>
          </aside>
        </section>
      </main>
  ...
  requireElement(selector) {
    ...
    if (!element) {
      throw new Error(
        `Required application element not found: ${selector}`,
      );
    }
