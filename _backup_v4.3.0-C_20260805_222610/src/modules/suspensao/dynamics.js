...
function readDynamicValues(controls) {
  return {
    ...
    contactMode:
      document.querySelector('#dynamic-contact-mode')?.value || 'continuous',
  };
}
...
function updateDynamicOutputs(outputs, values) {
  ...
  outputs['dynamic-road-amplitude'].value =
    `${formatNumber(values.roadAmplitude, 1)} mm`;

  outputs['dynamic-adhesion-index'].value =
    `${formatNumber(values.adhesionIndex, 0)}%`;
}
...
function updateDynamicMetrics(elements, metrics, evaluation) {
  ...
  const phaseLag = calculatePhaseLag(
    metrics.frequencyRatio,
    metrics.dampingRatio,
  );

  elements.phaseLag.textContent =
    `${formatNumber((phaseLag * 180) / Math.PI, 1)}°`;

  elements.bodyAmplitude.textContent =
    `${formatNumber(
...
function updateContactMetric(element, contactState) {
  ...
  element.dataset.state = contactState.inContact
    ? 'ok'
    : 'warning';
}
