...
function createOscillationWavePath(meanForce, variationPercent, width = 320, height = 92) {
  const safeMean = Number.isFinite(meanForce) && meanForce > 0 ? meanForce : 1;
  const safeVariation = Number.isFinite(variationPercent) ? Math.max(0, variationPercent) : 0;
  const amplitude = Math.min(height * 0.38, (safeVariation / 40) * height * 0.38);
  const center = height / 2;
  const points = [];

  for (let index = 0; index <= 48; index += 1) {
    const x = (index / 48) * width;
    const y = center - Math.sin((index / 48) * Math.PI * 4) * amplitude;
  ...
  return {
    path: points.join(' '),
    maximum: safeMean,
    minimum: safeMean * (1 - safeVariation / 100),
  };
}

function renderOscillationChart(serviceBrake, unit) {
  const wheels = [
    ['frontLeft', 'Dianteira esquerda', serviceBrake.frontAxle.forces.left],
    ['frontRight', 'Dianteira direita', serviceBrake.frontAxle.forces.right],
    ['rearLeft', 'Traseira esquerda', serviceBrake.rearAxle.forces.left],
    ['rearRight', 'Traseira direita', serviceBrake.rearAxle.forces.right],
  ];

  return `
    <section class="brake-result__section brake-result__oscillation-chart" aria-labelledby="brake-oscillation-chart-title">
      <div class="brake-result__oscillation-chart-header">
        <div>
          <h4 id="brake-oscillation-chart-title">Efeito da oscilação durante a rotação</h4>
          <p>Representação didática de duas voltas da roda. Quanto maior a variação, maior a diferença entre os picos e vales da força de frenagem.</p>
        </div>
          ...
          return `
            <article class="brake-result__wave-card ${stateClass}">
              <div class="brake-result__wave-card-header">
                ...
                <span>${formatNumber(variation, 0)}%</span>
              </div>
              <svg viewBox="0 0 320 92" role="img" aria-label="Oscilação de ${escapeHtml(label)}: ${formatNumber(variation, 0)} por cento">
                <line x1="0" y1="46" x2="320" y2="46" class="brake-result__wave-reference" />
                <path d="${wave.path}" class="brake-result__wave-line" />
              </svg>
              <div class="brake-result__wave-values">
                <span>Máx. ${formatNumber(wave.maximum)} ${escapeHtml(unit)}</span>
...
function renderScopedStyles() {
  return `
    <style>
      ...
      #${SECTION_ID} .brake-result__fixed-overview .brake-result__metric span {
        grid-area: label;
        font-size: 0.72rem;
        line-height: 1.15;
      }
      ...
      #${SECTION_ID} .brake-result__fixed-overview .brake-result__metric small {
        grid-area: status;
        align-self: end;
        font-size: 0.7rem;
        line-height: 1.05;
      ...
      #${SECTION_ID} .brake-result__oscillation-chart-header {
        display: flex;
        align-items: flex-start;
        justify-content: space-between;
        gap: 1rem;
      ...
      #${SECTION_ID} .brake-result__oscillation-chart-header p { margin: 0.25rem 0 0; }
      #${SECTION_ID} .brake-result__oscillation-limit {
        flex: 0 0 auto; padding: 0.35rem 0.55rem; border-radius: 999px;
        background: rgba(15, 23, 42, 0.08); font-size: 0.78rem; font-weight: 700;
      }
      #${SECTION_ID} .brake-result__wave-grid {
        display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 0.7rem;
      }
      #${SECTION_ID} .brake-result__wave-card {
        padding: 0.7rem; border: 1px solid rgba(148, 163, 184, 0.45);
        border-radius: 0.8rem; background: rgba(255, 255, 255, 0.72);
      }
      #${SECTION_ID} .brake-result__wave-card-header,
      #${SECTION_ID} .brake-result__wave-values { display: flex; justify-content: space-between; gap: 0.7rem; }
      #${SECTION_ID} .brake-result__wave-card-header span { font-weight: 800; }
      #${SECTION_ID} .brake-result__wave-card svg { display: block; width: 100%; height: 4.5rem; margin: 0.35rem 0; overflow: visible; }
      #${SECTION_ID} .brake-result__wave-reference { stroke: rgba(100, 116, 139, 0.45); stroke-width: 1; stroke-dasharray: 5 5; }
      #${SECTION_ID} .brake-result__wave-line { fill: none; stroke: currentColor; stroke-width: 4; stroke-linecap: round; stroke-linejoin: round; }
      #${SECTION_ID} .brake-result__wave-card.is-approved { color: #166534; }
      #${SECTION_ID} .brake-result__wave-card.is-attention { color: #a16207; }
      #${SECTION_ID} .brake-result__wave-card.is-failed { color: #b91c1c; }
      #${SECTION_ID} .brake-result__wave-values { color: #334155; font-size: 0.76rem; }
      #${SECTION_ID} .brake-result__wave-note { margin: 0.8rem 0 0; font-size: 0.86rem; }

      @media (max-width: 62rem) {
        #${SECTION_ID} .brake-result__wave-grid { grid-template-columns: 1fr; }
        #${SECTION_ID} .brake-result__oscillation-chart-header { flex-direction: column; }
        #${SECTION_ID} .brake-simulator__section-header {
        background: linear-gradient(135deg, #172554, #1d4ed8);
        color: #fff;
        border: 0;
  ...
  return `
    <div class="brake-result">
      ...
      <div class="brake-result__scroll-details" tabindex="0" aria-label="Detalhes complementares da análise">
        ...
        ${renderOscillationChart(service, unit)}
...
export function initializeFrenagemSimulator(_module, root) {
  ...
  const scheduleLiveUpdate = () => {
    ...
    liveUpdateFrame = window.requestAnimationFrame(() => {
      const result = simulateBrakeInspection(readFormInput(form), {
