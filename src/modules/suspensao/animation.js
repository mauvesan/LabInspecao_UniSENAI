...
const MAX_VISUAL_BODY_DISPLACEMENT = 34;
...
export function initializeSuspensionAnimation(root, options = {}) {
  ...
  const onContactStateChange =
    typeof options.onContactStateChange === 'function' ? options.onContactStateChange : () => {};
...
function drawAnimation(svg, metrics, time, dynamicState) {
  ...
  const bodyResponse = transmissibility * Math.sin(simulationPhase - phaseLag);

  const rawBodyAmplitude = bodyResponse * roadAmplitude;

  const visualBodyDisplacement = calculateVisualBodyDisplacement({
    rawBodyAmplitude,
    roadAmplitude,
    frequencyRatio,
    visualProfile: metrics.visualProfile,
  });
  ...
  appendText(
    ...
    contactState.inContact
      ? 'dyn-contact-status dyn-contact-ok'
      : 'dyn-contact-status dyn-contact-lost',
    'end',
  );
...
function drawRoadMarkers(svg, phase, amplitude) {
  ...
  const horizontalOffset =
    ((((phase / (2 * Math.PI)) * ROAD_WAVELENGTH) % markerSpacing) + markerSpacing) % markerSpacing;

  for (let baseX = -markerSpacing; baseX <= VIEWBOX_WIDTH + markerSpacing; baseX += markerSpacing) {
...
function resolveWheelMotion({
  ...
  const severity = clamp((excitationFrequency / 5) * (roadAmplitude / 30) * (1 - adhesion), 0, 1);
...
function calculateVisualBodyDisplacement({
  rawBodyAmplitude,
  roadAmplitude,
  frequencyRatio,
  visualProfile,
